	...
	private boolean isAbleToMoveAfterCrachCheck(TetrisBoard board, int offsetX, int offsetY) {
		...
		*/
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				if(block[i][j]>=1) {
					if(posX+j>=TetrisBoard.x) {						//오른쪽 경계 벗어남 판단
						return false;
					}
					if((posY+offsetY)+i>=TetrisBoard.y) {			//아랫쪽 바닥 판단
						controller.createNewBlock();
						return false;
					}
					if(board.block[(posY+offsetY)+i][(posX+offsetX)+j]*block[i][j]>=1) {	//맵상의 블록과 겹침 판단
						if(offsetY==1)
							controller.createNewBlock();
						return false;
					}
		...
		/*if (posY + getDownBorder() >= TetrisBoard.y) { // 바닥과 충돌 검사.
			controller.createNewBlock();
			return false;
		}*/
		return true;
